...
func (r *Resource) Describe(symbol string) (string, string, error) {
	...
	for _, s := range symbols {
		...
		if dsc, ok := dsc.(*desc.MessageDescriptor); ok {
			...
			_, formatter, err := grpcurl.RequestParserAndFormatter("json", r.descSource, nil, grpcurl.FormatOptions{
...
func (r *Resource) Invoke(ctx context.Context, metadata []string, symbol string, in io.Reader) (string, time.Duration, error) {
	...
	rf, formatter, err := grpcurl.RequestParserAndFormatter("json", r.descSource, in, grpcurl.FormatOptions{
...
func (r *Resource) AddProtos(protos []Proto) error {
	protoPath := filepath.Join(BasePath, r.clientConn.Target())
	err := os.MkdirAll(protoPath, 0o777)
	if os.IsExist(err) {
	...
	for _, proto := range protos {
		...
		if strings.HasSuffix(proto.Name, ".protoset") {
			...
			err = ioutil.WriteFile(filepath.Join(protoPath, "/", proto.Name), proto.Content, 0o777)
		} else {
			protoSlice = append(protoSlice, proto)
			err = ioutil.WriteFile(filepath.Join(protoPath, "/", proto.Name), prepareImport(proto.Content), 0o777)
		}
		if err != nil {
			return err
		}
